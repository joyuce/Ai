 ...
 * @author chenjing
 */
public class AnnClassifier implements Serializable {
    ...
    private void updateWeights(float eta) {
        ...
        for (int i = 0; i < mInputCount; i++) {
            for (int j = 0; j < mHiddenCount; j++) {
                mInputHiddenWeight[i][j] -=
                        eta * mInputNodes.get(i).getForwardOutputValue() * mHiddenNodes.get(j).getBackwardOutputValue();
            }
        }
        // 更新隐层到输出层的权重矩阵
        for (int i = 0; i < mHiddenCount; i++) {
            for (int j = 0; j < mOutputCount; j++) {
                mHiddenOutputWeight[i][j] -= eta * mHiddenNodes.get(i).getForwardOutputValue() * mOutputNodes.get(j)
                                                                                                             .getBackwardOutputValue();
            }
        }
    }
